...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        $this->leave = Leave::with('user', 'type')->where('status', 'approved')
         ...
         $this->interviewSchedule = InterviewCandidateSchedule::where('interview_date', date('Y-m-d'))->get();
       
         if(isset($this->interviewSchedule)){
             foreach($this->interviewSchedule as $key => $interview){
                 $this->interviewSchedule[$key]->candidateName = InterviewCandidate::where('id',$interview->interview_candidate_id)->where('move_candidate',0)->first();
             }
         }
